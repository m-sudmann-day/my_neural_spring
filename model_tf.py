...
class model_tf():
    ...
    def create_model(self, dh, guided_backprop):
        ...
        self.tensor_use_guided_relu = tf.placeholder(tf.bool, None, name='use_guided_relu')
    ...
    def get_tensor_accuracy(self):
        return (tf.get_default_graph().get_tensor_by_name('evaluation/accuracy:0'))
    
    def get_tensor_cost(self):
        return (tf.get_default_graph().get_tensor_by_name('cost_/cost:0'))
    ...
    def get_tensor_Z(self):
        if 'Z:0' in tf.get_default_graph().as_graph_def().node:
            return tf.get_default_graph().get_tensor_by_name('Z:0')
        else:
            return None
    ...
    def train_normal(self, dh):
        ...
        #with tf.device('/gpu:0'):
            with tf.Session() as sess:
                ...
                summary_writer = tf.summary.FileWriter('../tboard/', sess.graph)
    
                log = {'loss':[], 'acc':[], 'val_loss':[], 'val_acc':[]}
    
                # Temporary hack.  Enables us to print out an untrained result at the beginning. (epoch 0).
                ...
                while self.num_remaining_epochs > 0:
                    ...
                    if self.num_completed_epochs > 0:
                        ...
                        if self.num_completed_epochs % 10 == 0:              
                             self.save_variables(sess, 'epoch_' + str(self.num_completed_epochs))
    ...
    #                     self.num_remaining_epochs = int(answer)
                ...
                self.save_variables(sess, 'final')
                self.plot_training_log(log)
...
#         vis2_final.show()
    ...
    def get_next_batch_as_feed_dict(self, dh, use_guided_relu):
        ...
        feed_dict[self.tensor_use_guided_relu] = use_guided_relu
...
#                 imsave(os.path.join(path, 'w' + str(filter) + '.png'), w_filter)
    ...
    def save_variables(self, sess, folder_name):
